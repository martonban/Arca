...
bool Arca::ArcaIO::IsFileExists(const std::filesystem::path& fullFilePath) {
    return std::filesystem::exists(fullFilePath) && std::filesystem::is_regular_file(fullFilePath);
}

bool Arca::ArcaIO::CreateFolder(const std::filesystem::path& path, const std::string& name) {
...
bool Arca::ArcaIO::IsFolderExists(const std::filesystem::path& path) {
    try {
